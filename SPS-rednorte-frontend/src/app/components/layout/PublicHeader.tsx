...
export function PublicHeader({ onLogin, onReserva, onUrgencia }: PublicHeaderProps) {
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center justify-end gap-5 px-6 py-1.5 bg-[#023e8a] text-white text-xs rounded-t-xl">
        ...
        <button onClick={onUrgencia} className="hover:text-cyan-300 transition">Urgencias 24/7</button>
        <span className="opacity-40">|</span>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t px-4 py-4 flex flex-col gap-2 bg-white rounded-b-xl">
          ...
          <button 
            onClick={onUrgencia}
            className="mt-2 w-full py-2 rounded-full border border-[#e63946] text-[#e63946] font-bold text-sm">
            Consultar Urgencia
          </button>
