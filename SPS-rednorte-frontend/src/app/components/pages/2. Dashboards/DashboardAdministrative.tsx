...
export function DashboardAdministrative() {
  ...
  const handleIngresoUrgencia = async () => {
    try {
      ...
      toast.success('Paciente ingresado a Urgencias', { description: res });
      setShowUrgenciaModal(false);
      setRutIngreso('');
      setMotivo('');
    } catch (error) {
      toast.error('Error al registrar ingreso en Urgencias');
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button 
            ...
            className="flex items-center gap-2 bg-[#e63946] hover:bg-[#c1121f] text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-md transition-all">
            <Siren className="h-4 w-4" /> Ingreso Urgencia
          </button>
      ...
      {showUrgenciaModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <Card className="w-full max-w-lg shadow-xl border-none animate-in fade-in zoom-in-95">
            <CardHeader className="bg-slate-50 border-b border-slate-100 rounded-t-xl flex flex-row items-center justify-between">
              <CardTitle className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <Siren className="h-5 w-5 text-[#e63946]" /> Ingreso a Urgencias
              </CardTitle>
              <button onClick={() => setShowUrgenciaModal(false)} className="text-slate-400 hover:text-slate-600">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full border border-slate-200 rounded-lg p-2.5 focus:ring-2 focus:ring-[#e63946]/20 focus:border-[#e63946] outline-none" 
                  rows={3} 
                  placeholder="Ej: Fuerte dolor abdominal y fiebre" 
                />
              </div>
              <button onClick={handleIngresoUrgencia} className="w-full bg-[#e63946] hover:bg-[#c1121f] text-white font-bold py-3 rounded-lg transition-colors mt-2">
                Registrar y Enviar a Triage
              </button>
            </CardContent>
          </Card>
        </div>
