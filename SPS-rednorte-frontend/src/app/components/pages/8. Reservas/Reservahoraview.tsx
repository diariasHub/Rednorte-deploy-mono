...
export function Reservahoraview({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex gap-6 h-full w-full">
      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6 overflow-y-auto relative">
        <button 
          onClick={onBack} 
          className="absolute top-4 right-4 flex items-center gap-2 text-slate-500 hover:text-[#0b3c5d] bg-slate-50 hover:bg-slate-100 px-3 py-2 rounded-lg text-sm font-semibold transition-colors"
        >
